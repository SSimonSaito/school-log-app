...
try:
    ...
    period = "MHR"
    st.markdown("🖌️ 本アプリでは朝のホームルーム（MHR）の出欠のみを記録します。")
    ...
    st.markdown("## ✏️ 出欠入力")
    status_options = ["○", "／", "公", "病", "事", "弊", "停", "遅", "早", "保"]
    attendance_data = []
    alerts = []
    ...
    if st.button("🗓 出欠を一括登録"):
        ...
        try:
            ...
            columns = ["date", "timestamp", "class", "student_id", "student_name", "status", "teacher", "period", "memo"]
            updated_df = pd.concat([
                attendance_df[columns],
                pd.DataFrame(enriched_rows, columns=columns)
            ], ignore_index=True)
