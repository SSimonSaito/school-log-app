...
st.markdown(f"## 🏫 {homeroom_class} 出欠＋メモ入力（MHR）")
...
for _, row in students_in_class.iterrows():
    ...
    with col1:
        status = st.radio(f"{student_name}（{student_id}）", status_options, horizontal=True,
                          index=status_options.index(default_status), key=f"status_{student_id}")
    with col2:
...
if st.button("📅 出欠を登録"):
    ...
    enriched_rows = [
        [today_str, now, homeroom_class, row["student_id"], row["student_name"], row["status"], teacher_name, period, row["memo"]]
